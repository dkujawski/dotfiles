...
# Function to attempt op signin to fox account
attempt_fox_signin() {
    debug_log "Attempting op signin to fox account"
    ...
    if [ -t 0 ] && [ -t 1 ] && [ -t 2 ]; then
        ...
        if op signin --account foxcorporation.1password.com >/dev/null 2>&1; then
            debug_log "Successfully signed in to fox account"
            return 0
        else
            debug_log "Signin to fox account failed or was cancelled"
            return 1
        fi
    else
...
if attempt_load_secrets 30; then
    ...
    if [ $exit_code -eq 124 ]; then
        ...
        debug_log "Initial secret loading failed (exit code: $exit_code), attempting op signin to fox account"
        
        # Attempt op signin to fox account
        if attempt_fox_signin; then
            debug_log "Fox account signin successful, retrying secret loading"
            
            # Retry loading secrets after successful signin
            if attempt_load_secrets 15; then
                debug_log "Secrets loaded successfully after fox account signin"
            else
                retry_exit_code=$?
